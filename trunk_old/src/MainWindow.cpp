...
void MainWindow::updateImageDone( bool havenew ) {
  if ( havenew ) {
    settings->setValue( "apod:lastmodified", getter->getLastModified() );
    settings->sync();
    qDebug() << "sett_lastmod=" << qPrintable( settings->value( "apod:lastmodified" ).toString() );
    QDateTime now = QDateTime::currentDateTime();
    QString fn = settings->value( "imagelocation" ).toString() + "/" + now.toString( FILEFORMAT ) + settings->value( "sourcetype" ).toString();
